...
export default function SendMessage({ message, onComplete, type='messages' }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        `${process.env.REACT_APP_API_URL}api/admin/${type}/${message.id}/reply/`,
        formData
      );
      onComplete()

      addToast("Email Send Successfully", { appearance: "success" });
    } catch (error) {
      addToast("Failed to send messages", { appearance: "error" });
    }
  };
