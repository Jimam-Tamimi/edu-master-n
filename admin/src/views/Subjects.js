...
export default function Subjects({ color }) {
  ...
  const getSubjects = async () => {
    try {
      ...
      addToast('Failed to get subjects', { appearance: 'error' });
  ...
  const addSubject = async subjectName => {
    ...
    try {
      const res = await axios.post(`${process.env.REACT_APP_API_URL}api/admin/subjects/`, { subject: subjectName })

      setSubjects(prevD => ([res.data, ...prevD]))

      addToast('Subject added successfully', { appearance: 'success' });

    } catch (error) {
      addToast('Failed to add subject', { appearance: 'error' });

    }
  }
  const deleteSubject = async id => {
    try {
      const res = await axios.delete(`${process.env.REACT_APP_API_URL}api/admin/subjects/${id}/`)

      setSubjects(prevD => prevD.filter(s => s.id != id))

      addToast('Subject removed successfully', { appearance: 'success' });

    } catch (error) {
      addToast('Failed to remove subject', { appearance: 'error' });
          ...
          <div className="flex flex-wrap items-center">
            ...
            <form onSubmit={filterTutors} className="md:flex flex-row flex-wrap items-center lg:ml-auto mr-3">
              <div className="relative flex w-full flex-wrap items-stretch my-2">
                ...
                <Button onClick={e => { addSubject(window.prompt("Enter Subject Name")) }} >Add Subject</Button>
              </div>
...

              {
                subjects?.map(subject => (
                  <tr>
                    ...
                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                      <ShowMore onClick={(e) => window.confirm('Are you sure you want to remove this subject?') ? deleteSubject(subject.id) : ''}>
                        Remove
                      </ShowMore>
                    </td>
                  </tr>
